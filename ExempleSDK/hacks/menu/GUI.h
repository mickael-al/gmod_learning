...
#endif
				ImGui::CustomSpacing(9.f);
				ImGui::PushItemWidth(80.f); // bouton largeur
				const float spacing = 5.0f;
				const float buttonWidth = 100.0f;
				const float buttonHeight = 0.0f; // Auto height

				// Première ligne
				if (ImGui::Button("Execute script", ImVec2(buttonWidth, buttonHeight))) {
					std::thread(ExecuteScript, Settings::ScriptInput).detach();
				}

				ImGui::SameLine();
				ImGui::SetCursorPosX(ImGui::GetCursorPosX() + spacing); // Espacement horizontal

				if (ImGui::Button("Load from file", ImVec2(buttonWidth, buttonHeight))) {
					std::thread(LoadScriptFromFile).detach();
				}

				// Seconde ligne
				ImGui::Dummy(ImVec2(0.0f, spacing)); // Espacement vertical

				if (ImGui::Button("Mirror Bhop", ImVec2(buttonWidth, buttonHeight))) {
					std::thread(LoadMirror).detach();
				}

				ImGui::SameLine();
				ImGui::SetCursorPosX(ImGui::GetCursorPosX() + spacing); // Espacement horizontal

				if (ImGui::Button("Custom Option", ImVec2(buttonWidth, buttonHeight))) {
					// Action bouton 4
				}

				ImGui::PopItemWidth();

				ImGui::SameLine(345.f); 
				ImGui::PushItemWidth(158.f); 
				ImGui::SetCursorPosY(ImGui::GetCursorPosY() + 5.f);
				ImGui::Combo("Lua State", &Globals::executeState, executorLuaState, IM_ARRAYSIZE(executorLuaState));
				ImGui::PopItemWidth();
