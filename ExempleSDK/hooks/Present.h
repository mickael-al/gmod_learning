...
HRESULT __stdcall hkPresent(IDirect3DDevice9* pDevice, CONST RECT* pSourceRect, CONST RECT* pDestRect, HWND hDestWindowOverride, CONST RGNDATA* pDirtyRegion)
{
	...
	ImGui::BeginMenuBackground("Credits window", nullptr, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoTitleBar , 0.3f);
	{
		...
		ImGui::NewLine();
		ImGui::SameLine(15.f);
		ImGui::Text(std::string("Eminence in HTTPS " + std::string(CheatVersion)).c_str());
	}
	ImGui::End();	
	*/
